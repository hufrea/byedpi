...
------
### Я запустил, а ничего не работает
В отличие от Goodbyedpi, который работает сам по себе, вмешиваясь в работу операционной системы с сетью на самом низком уровне, Byedpi есть локальный прокси-сервер, и его надо использовать в необходимых вам программах.

Скажем, в браузере Firefox для этого необходимо открыть Настройки, пролистнуть в самый низ, нажать кнопку Настроить в разделе Настройки сети, нажать на четвёртый сверху переключатель Ручная настройка прокси
и вписать в Узел SOCKS строку 127.0.0.1, а в Порт значение 1080, если вы не меняли его параметром -p. 

Если после запуска видео на youtube всё равно не подгружаются, то в разделе Discussions этого пакета есть разные наборы ключей, которые у кого-то заработали.
